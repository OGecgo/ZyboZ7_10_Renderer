...
static inline float  deg2radf(float  deg) { return deg * DEG2RAD_F; }
...
static inline Vec2  vec2_lerp   (Vec2 a, Vec2 b, float t) { Vec2 r;   r.x = a.x + t * (b.x - a.x);   r.y = a.y + t * (b.y - a.y);   return r;}
